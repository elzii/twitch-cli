...
function parseStreams(query, callback) {
  ...
  searchStreams(query, function (streams) {
    ...
    inquirer.prompt([{
      type: "input",
      name: "stream_number",
      message: chalk.green.bold("Choose stream (1-"+streams.length+")"),
      validate: function( value ) {
        if ( value > streams.length ) {
          return "Please enter a valid number (1-"+streams.length+")";
        } else if( !value ) {
          return "Please enter a valid number (1-"+streams.length+")";
        } else if( value == "b" ) {
          return true;
...
function parseChannels(query, callback) {
  ...
  searchChannels(query, function (channel) {
    ...
    inquirer.prompt([{
      type: "input",
      name: "channel_number",
      message: chalk.green.bold("Choose stream (1-"+channel.length+")"),
      validate: function( value ) {
        if ( value > channel.length ) {
          return "Please enter a valid number (1-"+channel.length+")";
        } else if( !value ) {
          return "Please enter a valid number (1-"+channel.length+")";
        } else if( value == "b" ) {
          return true;
